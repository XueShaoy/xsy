...
前文默认「每人本地跑 CLI + 可选 daemon」。当团队规模扩大、需要统一索引版本、或 Agent 需要远程调用检索时，可以把检索层部署为**团队共享的只读服务**。

### 10.1 为什么要上线

| 痛点（纯本地） | 上线后 |
|----------------|--------|
| 每人 clone 后都要 `build_index.py`、下载 embedding 模型 | 服务侧统一构建，客户端零索引维护 |
| 不同人本地索引版本可能不一致 | 索引与 catalog `git_sha` 绑定，全员同一版本 |
| Agent 只能在本机调 `search.py` | HTTP API 可被 IDE / CI / 其他服务远程调用 |
| 冷启动 ~20s 每人各付一次 | 模型常驻内存，首请求即亚秒级 |

**边界不变**：上线的是**检索路由服务**，不是 SQL 执行引擎。服务只返回 `path` / `metric_id` / `reject` / `ambiguous`，不执行 SQL、不改仓库、不接触业务库。

### 10.2 部署形态演进

```mermaid
flowchart LR
    A["阶段1<br/>本地 CLI<br/>search.py"] --> B["阶段2<br/>本机 daemon<br/>127.0.0.1:8765"]
    B --> C["阶段3<br/>团队共享服务<br/>内网 HTTP API"]
    C --> D["阶段4 可选<br/>多副本 + 负载均衡"]
```

| 阶段 | 适用 | 特点 |
|------|------|------|
| 本地 CLI | 个人开发、调试 | 零依赖，每次冷启动 |
| 本机 daemon | 单人频繁调 Agent | 模型常驻，热重载索引 |
| 团队共享服务 | 3～50 人团队 | 统一索引、远程调用、鉴权 |
| 多副本 | 高可用 / 高峰并发 | 无状态水平扩展 |
...
flowchart TB
    ...
    subgraph service [检索服务 只读]
        API["FastAPI + uvicorn"]
        Engine["混合检索引擎<br/>BM25 + FAISS"]
        Model["embedding 模型常驻"]
    end

    subgraph artifacts [索引制品 由 CI 构建]
        Chunks["chunks.jsonl"]
        BM25["bm25.pkl"]
        Vec["vectors.faiss"]
        Manifest["manifest.yaml"]
    ...
    Build --> Publish
    Publish --> artifacts
    artifacts -->|启动时拉取| service
```

核心原则：

- **服务无状态**：索引文件从制品库/对象存储挂载或启动时下载，不在容器内写业务数据
- **版本可追溯**：`manifest.yaml` 含 `git_sha`、`built_at`、`embedding_model`，客户端响应带回 `manifest_git_sha`
- **降级可用**：向量模型加载失败时自动 BM25-only，与本地行为一致

### 10.4 关键组件与选型

| 组件 | 本地现状 | 线上建议 |
|------|----------|----------|
| **HTTP 服务** | `serve.py` 单线程 `ThreadingHTTPServer` | FastAPI + uvicorn（或 gunicorn + uvicorn worker） |
| **embedding 模型** | 首次从 HF 镜像下载 ~100MB | **打入 Docker 镜像**，启动零外网依赖 |
| **索引分发** | `catalog/search/` 本地生成 | CI 构建 → 对象存储/制品库版本化，与 git tag 或 commit sha 绑定 |
| **向量检索** | FAISS `IndexFlatIP`（≤1000 条） | 当前规模继续暴力检索即可；上万条再考虑 IVF/HNSW 或托管向量库 |
| **并发** | 单进程模型缓存 `_MODEL_CACHE` | 每 worker 一份模型（内存换延迟）；或单 worker + 异步队列 |
| **配置** | `project.yaml` | 环境变量 + ConfigMap / 密钥管理，敏感项不入镜像 |

### 10.5 部署方式对比

| 方式 | 复杂度 | 适用场景 | 注意点 |
|------|--------|----------|--------|
| **单机 daemon + systemd** | 低 | 小团队内网一台 Linux | 绑定内网 IP，防火墙仅开放给办公网/VPN |
| **Docker + Nginx 反代** | 中 | 需要 HTTPS、多环境（测试/生产） | 镜像内置模型；健康检查 `GET /health` |
| **K8s Deployment** | 高 | 需高可用、滚动发布、HPA | 索引用 PVC 或 init 容器拉制品；滚动时新旧 Pod 索引版本一致 |
| **Serverless / 函数计算** | — | **不推荐** | 冷启动加载模型 10～20s，与「常驻低延迟」目标冲突 |

**推荐路径**：小团队从 **Docker + 内网反代** 起步；规模上来再迁 K8s，检索 API 契约不变。

### 10.6 索引更新与热发布

```mermaid
flowchart LR
    MR["catalog / SQL 头部 MR"] --> Merge["合并 main"]
    Merge --> CI["CI: build_index.py"]
    CI --> Artifacts["上传索引制品<br/>版本 = git_sha"]
    Artifacts --> Notify["通知/轮询新版本"]
    Notify --> Pull["服务拉取或挂载新索引"]
    Pull --> Reload["热重载 manifest<br/>或滚动重启 Pod"]
    Reload --> Live["全实例同一 manifest_git_sha"]
```

操作要点：

1. **构建与发布分离**：MR 合并触发 CI 跑 `build_index.py`，产物上传制品库（不提交大二进制进 Git 亦可，至少提交 `chunks.jsonl` + `manifest.yaml` 供 diff）
2. **服务拉取策略**：启动时拉取 `latest` 或指定 `git_sha`；运行中监听 `manifest.yaml` mtime（复用第 7 章 daemon 热重载思路）
3. **滚动发布**：K8s 滚动更新时，确保新 Pod 拉齐新索引后再接流量；旧 Pod Drain 前仍服务旧版本无妨（短时间双版本可接受，以 `manifest_git_sha` 区分）
4. **回滚**：制品按 commit sha 保留历史版本，一键回滚到上一索引包

### 10.7 安全与运维

| 项 | 建议 |
|----|------|
| **网络** | 仅内网或 VPN；公网暴露必须 HTTPS + 鉴权 |
| **鉴权** | API Key / mTLS / 内网 SSO 网关；`POST /search` 需带 Token |
| **权限** | 服务**只读**；禁止任意 SQL 执行、禁止写 catalog |
| **健康检查** | `GET /health` 返回 `status`、`model`、`built_at`、`manifest_git_sha` |
| **监控** | QPS、P99 延迟、`reject` 率、`ambiguous` 率、索引版本漂移告警 |
| **漂移告警** | 线上 `manifest_git_sha` ≠ 仓库 HEAD 超过 N 小时 → 通知维护者重建/发布 |

### 10.8 数据安全

- 索引内容仅为 **`search_text`**（catalog 元数据 + SQL 头部口径），**不是** SQL 正文全文
- embedding 在**服务内网** encode，query 文本不出域（若合规要求极高，可仅 BM25 模式）
- 不向公有云上传标准 SQL 文件；制品库存储在内网对象存储
- 响应 JSON 只含 `path`、`metric_id` 等路由信息，不含可执行 SQL 正文

### 10.9 上线检查清单

部署前逐项确认：

- [ ] Docker 镜像已内置 embedding 权重，启动无需访问外网 HuggingFace
- [ ] 索引制品版本与 catalog 当前 `git_sha` 一致
- [ ] `GET /health` 返回 `has_vectors: true`（或接受 BM25-only 降级并已文档化）
- [ ] `POST /search` 冒烟：已知 query 返回预期 `metric_id`，`reject: false`
- [ ] 鉴权已开启，未授权请求返回 401
- [ ] 监控与日志已接入（至少记录 query、score、reject、latency）
- [ ] 回滚预案：上一版索引制品可一键切换
- [ ] Agent / CLI 已配置服务 Base URL，与本地 `search.py` 输出 JSON 格式一致
